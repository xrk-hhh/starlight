...
const versions = [
  {
    ...
    note: '浅色主题星空彻底修复：新增 FlatStarfield（2D Canvas 墨点星野）专供浅色主题——加色混合的 WebGL 星海在浅底加光即隐形，而透明画布 NormalBlending 多方案实测在部分环境零输出（预乘三件套/朴素组合均不可见），改用任何环境都稳定可见的 2D 渲染；深色主题 WebGL 星海原样保留，浅色时暂停其渲染循环省 GPU / 深浅切换即时无缝 / 移动端与密度 off 同规则联动 / 支持音乐呼吸（audioLevel）与 reduced-motion 静帧 / 像素级验证：樱庭 833 星、纸笺 749 星（此前均为 0），深色往返无回归',
  },
  {
    v: 'v2.13.0',
    date: '2026-08',
    note: '全站体检：移动端 8 页 + 文章页（表格/代码块）零横向溢出，五主题视觉、首页/关于数据组件（今日一题·贡献星图·技能星云·OJ）、快捷键、桌宠、电台、回到顶部全部复验通过 / 主题系统引入 light 标志（当日的混合模式切换方案在部分环境无效，由 v2.13.1 的 FlatStarfield 完成修复）',
  },
  {
    v: 'v2.12.1',
    date: '2026-08',
