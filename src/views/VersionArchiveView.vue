...
const versions = [
  {
    ...
    note: '星空架构回归 v2.11 认证形态：用户确认 v2.11 全主题背景正常——真实浏览器的预乘画布合成下，加色粒子在浅色主题同样可见（合成结果 = color·alpha + bg·(1-alpha)）。撤回 v2.13.0/2.13.1 的混合模式切换与 FlatStarfield 双层实验（后者的「浅色不可见」结论源自内嵌测试浏览器的合成怪癖，不适用于真实浏览器），ParticleBackground 与 v2.11 逐字节一致（仅保留 v2.12 流星彩蛋事件），浅色 canvas 透明度恢复 0.35',
  },
  {
    v: 'v2.13.1',
    date: '2026-08',
